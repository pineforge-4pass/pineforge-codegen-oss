...
public:
    ...
    void on_bar(const Bar& bar) override {
        ...
        // Pine v6 lazy operand: TA state advances every bar, only the value is gated.
        const auto _pf_every_bar_ta_1 = (history_advances_new_bar() ? _ta_crossover_7.compute(lam, lamSma) : _ta_crossover_7.recompute(lam, lamSma));
        if ((((covReady && !(is_na(lam))) && !(is_na(lamSma))) && _pf_every_bar_ta_1)) {
            strategy_entry(std::string("Long"), true, na<double>(), na<double>(), na<double>(), "");
        }
        // Pine v6 lazy operand: TA state advances every bar, only the value is gated.
        const auto _pf_every_bar_ta_2 = (history_advances_new_bar() ? _ta_crossunder_8.compute(lam, lamSma) : _ta_crossunder_8.recompute(lam, lamSma));
        if ((((covReady && !(is_na(lam))) && !(is_na(lamSma))) && _pf_every_bar_ta_2)) {
