...
└── codegen/
    ...
    ├── session_market.py           TU-local session.ismarket helper: the
    │                               engine's session calendar asked at a
    │                               chart bar's open time.
...
| Session flags | A chart bar's `session.ismarket` asks the engine's session calendar (`native_calendar::session_day_at`, on the calendar the kernel builds from the same session and timezone) whether it holds the bar's open time; a D/W/M bar is in market. TradingView flags a bar by its open time (on extended-hours NASDAQ:AAPL 60 the 09:00 bar, which holds the 09:30 open, is pre-market and the 16:00 bar post-market) and reads a day mask per session day. The emitted `_pf_session_ismarket` helper (`codegen/session_market.py`) replaced the time-of-day predicate, which missed every Sunday-evening open under a `:23456` mask and every `0000-2400` bar; the kernel's per-bar `session_ismarket_` asks the bar's grid interval and puts that 16:00 bar in market. A bar in market is in neither `session.ispremarket` nor `session.ispostmarket`; off it, the 04:00-to-open and close-to-20:00 windows decide (the predicates read 98-117 in-market bars of each ES1! / XAUUSD tape as pre-market). Inside a `request.security` payload the three keep the predicates at the security bar's open time and WARN. `session.isfirstbar` / `islastbar` (and `_regular`) read the kernel's session-day facts, unchanged; on the extended-hours tape they differ from TradingView (one session string, the grid interval), as the test pins. `tests/test_e2e_session_ismarket.py` replays 14 TradingView tapes (`tests/fixtures/session_ismarket`). |
