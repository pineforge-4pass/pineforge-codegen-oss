...
| Session flags | `session.ismarket` reads the kernel's in-session fact of the script bar (`NativeDecisionContext::in_session`, read off the session day the bar belongs to), which the host stores as `session_ismarket_` before every source callback, and a bar in market is in neither `session.ispremarket` nor `session.ispostmarket` (bars out of market keep the 04:00-to-open and close-to-20:00 windows); `session.isfirstbar` / `islastbar` (and `_regular`) read its session-day facts. The time-of-day predicates missed every Sunday-evening open under a `:23456` day mask and every `0000-2400` bar, and read in-market bars of an overnight session as pre- or post-market (98-117 bars of each ES1! / XAUUSD tape pre-market), where TradingView's tapes (`tests/fixtures/session_ismarket`) flag all 1,138 bars of six 60-minute charts in market and none in an extended session. A `request.security` payload, whose bars carry no host fact, and a batch of fewer than two bars given no timeframe (the engine presents no facts) keep the predicates. `tests/test_e2e_session_ismarket.py` replays every tape. |
