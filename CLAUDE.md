...
| Session flags | `session.ismarket` reads the kernel's in-session fact of the script bar (`NativeDecisionContext::in_session`, read off the session day the bar belongs to), which the host stores as `session_ismarket_` before every source callback; `session.isfirstbar` / `islastbar` (and `_regular`) read its session-day facts. The time-of-day predicate missed every Sunday-evening open under a `:23456` day mask and every `0000-2400` bar, where TradingView's tapes (`tests/fixtures/session_ismarket`) flag all 1,138 bars of six 60-minute charts in market. A `request.security` payload, whose bars carry no host fact, and a batch of fewer than two bars given no timeframe (the engine presents no facts) keep the predicate. `tests/test_e2e_session_ismarket.py` replays every tape. |
